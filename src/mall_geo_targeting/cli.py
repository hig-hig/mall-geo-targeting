...
def main() -> int:
    ...
    try:
        result = run(args.project_root.resolve(), data_mode=args.data_mode, accessibility_mode=args.accessibility_mode)
    except (OSError, KeyError, TypeError, ValueError) as exc:
        logging.getLogger(__name__).error("分析に失敗しました: %s", exc)
        return 1
    print(f"モード: {result['data_mode']}+{result['accessibility_mode']} / メッシュ: {result['mesh_count']} / スコア算出: {result['scored_count']} / 配信ゾーン: {result['delivery_zone_count']}")
