...
地図の施設相対選択指数と車・徒歩・自転車の到達条件付き選択指数は、0～100%を固定20ポイント刻みで5区分します。同じ色は全4指標で同じ数値範囲を表すため、交通手段を切り替えて比較できます。到達候補なし／No-dataは数値区分へ含めず、専用の灰色で表示します。総合スコア、人口、道路、商業POIなど、その他の実数指標は従来どおり現在表示中メッシュの五分位で分類し、フィルター変更時に区切りを再計算します。配信閾値は凡例区分とは別に、配信適格メッシュ内の上位20%から算出します。スコアが閾値以上でも必須データ条件を満たさないメッシュは配信ゾーンに含めません。
...
- 地図凡例：選択指数は固定20ポイント区分、その他の実数指標は表示中メッシュの五分位。将来UIで変更可能にする意図を記録
