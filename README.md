...
人口、競合施設、周辺環境、交通手段などの公開データと設定条件をもとに、商業施設・小売店舗・生活関連施設などの周辺地域をメッシュ単位で分析し、商圏と配信候補エリアを可視化します。

> 公開データと設定条件に基づく参考値であり、実際の来店者や広告成果を直接示すものではありません。未校正シナリオを含み、手段別指数は表示専用です。配信可否を保証するものではありません。
...
地図の総合スコアは0～100点を固定20点刻みで5区分し、配信閾値33.7点は色区分とは別に扱います。施設相対選択指数と車・徒歩・自転車の到達条件付き選択指数は、0～100%を固定20ポイント刻みで5区分します。同じ色は全4指標で同じ数値範囲を表すため、交通手段を切り替えて比較できます。到達候補なし／No-dataは数値区分へ含めず、専用の灰色で表示します。人口、道路、商業POIなど、その他の実数指標は従来どおり現在表示中メッシュの五分位で分類し、フィルター変更時に区切りを再計算します。配信閾値は凡例区分とは別に、配信適格メッシュ内の上位20%から算出します。スコアが閾値以上でも必須データ条件を満たさないメッシュは配信ゾーンに含めません。
...
- 地図凡例：総合スコアは固定20点区分、選択指数は固定20ポイント区分、人口・道路・商業POI等は表示中メッシュの五分位。配信閾値33.7点は色区分とは別。将来UIで変更可能にする意図を記録
- 交通手段割合：別概念として無効・未実装。架空の割合は生成しない

車・徒歩・自転車の到達条件付き選択指数は、施設への直線距離、手段別beta、距離別availabilityを使う表示専用シナリオです。対象施設と競合施設の各効用へavailabilityを掛けて正規化します。車は距離上限なしです。徒歩は「500mまで到達可能性100%、500m～2kmで段階減衰、2km以上を到達対象外とする未校正シナリオ」、自転車は「1kmまで到達可能性100%、1～4kmで段階減衰、4km以上を到達対象外とする未校正シナリオ」です。これらは実績値ではなく、担当者が変更可能な暫定値です。実経路、実所要時間、道路方向、勾配、待ち時間、実来館率を表さず、総合スコア、配信適格、配信ゾーンには使用しません。
...
- `mesh_id`: 対象施設を基準に作った独自250m正方形の識別子
